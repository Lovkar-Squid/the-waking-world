...
public final class Waker {
    ...
    public static ColossusEntity wake(ServerLevel level, LivingEntity caller, Palette palette, int height) {
        ...
        return wakeAt(level, from, palette, height, from.add(dir.scale(distance)), caller instanceof Player p ? p.getUUID() : null);
    }
    ...
    public static ColossusEntity wakeAt(ServerLevel level, LivingEntity caller, Palette palette, int height, Vec3 at) {
        return wakeAt(level, caller == null ? at.add(0, 0, -20) : caller.position(), palette, height, at, caller instanceof Player p ? p.getUUID() : null);
    }
    ...
    public static ColossusEntity wakeAt(ServerLevel level, Vec3 from, Palette palette, int height, Vec3 at) {
        return wakeAt(level, from, palette, height, at, null);
    }

    /**
     * Wakes one at a given spot, facing a point, for a player. A supporter's giant rises dressed in
     * the style they chose on the supporter service - the same shape, other blocks; nothing else differs.
     */
    public static ColossusEntity wakeAt(ServerLevel level, Vec3 from, Palette palette, int height, Vec3 at, UUID waker) {
        ...
        palette = SupporterCosmetics.dress(palette, waker);
